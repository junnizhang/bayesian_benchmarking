...
## kBaseDir <- "u:/SocPopStats/Population Statistics/SRS_Secure/John/Benchmarking/Smoking simulation"
## kOutDir <- "C:/Users/jbryant"
kBaseDir <- "/Users/johnbryant/Documents/NewZealandChina/benchmarking2/"
kCalcDir <- file.path(kBaseDir, "Paper - calculations")
kOutDir <- kCalcDir
kPaperDir <- file.path(kBaseDir, "Paper")

library(DemographicEstimation)
library(survey)
library(coda)
...
setwd(kOutDir)
d <- read.csv(results.file.diagnostics)
diagnostics <- c("accept.prob", "accept.bench",
                 "autocorr", "psrf", "sigma")
d <- reshape(d,
             varying = list(diagnostics),
             v.names = "value",
             idvar = c("iteration", "model.variant"),
             timevar = "diagnostic",
             times = diagnostics,
             direction = "long",
             drop = c("n.young.rich", "n.young.rich.smokes"))
rownames(d) <- NULL
d$diagnostic <- factor(d$diagnostic,
                       levels = diagnostics,
                       labels = c("pr accept - prob",
                           "pr accept - benchmarks",
                           "autocorr - prob",
                           "PSRF",
                           "sigma"))
d$model.variant <- factor(d$model.variant,
                          levels = c("Soft",
                              "Medium",
                              "Hard",
                              "None"))
p <- bwplot(model.variant ~ value | diagnostic,
            data = d,
            par.settings = list(box.rectangle = list(col = "black"),
                box.umbrella = list(col = "black"),
                plot.symbol = list(col = "black", pch = "|"),
                fontsize = list(text = 7, points = 7),
...
graphics.off()
setwd(kBaseDir)
pdf(file = "smoking_diagnostics.pdf", width = 6.5, height = 4)
plot(p)
dev.off()

sigma.table <- subset(d, subset = diagnostic == "sigma")
sigma.table$model.variant <- factor(sigma.table$model.variant,
                                    levels = c("None", "Hard", "Medium", "Soft"))
sigma.table <- tapply(sigma.table$value,
                      sigma.table["model.variant"],
                      mean)                      
sigma.table
...
setwd(kOutDir)
pc <- read.csv(results.file.perform.cell)
pb <- read.csv(results.file.perform.bench)
